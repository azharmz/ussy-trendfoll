...
class AlertStateTests(unittest.TestCase):
    ...
    def test_upgrade_to_actionable(self):
        previous = frame([{"symbol": "AAA", "date": "2026-09-13", "investability_status": "PASS", "tradability_status": "FAIL"}])
        latest = frame([{"symbol": "AAA", "date": "2026-09-14", "investability_status": "PASS", "tradability_status": "PASS", "close_raw": 11}])
    ...
    def test_lost_tradability(self):
        previous = frame([{"symbol": "AAA", "date": "2026-09-13", "investability_status": "PASS", "tradability_status": "PASS"}])
        latest = frame([{"symbol": "AAA", "date": "2026-09-14", "investability_status": "PASS", "tradability_status": "FAIL", "close_raw": 9}])
        events = compute_alert_transitions(latest, previous, universe(latest))
        self.assertEqual(len(events), 1)
    ...
    def test_invalidated_when_investability_fails(self):
        ...
        latest = frame([{"symbol": "AAA", "date": "2026-09-14", "investability_status": "FAIL", "tradability_status": "FAIL", "close_raw": 8}])
        events = compute_alert_transitions(latest, previous, universe(latest))
        self.assertEqual(len(events), 1)
    ...
    def test_unchanged_state_emits_nothing(self):
        previous = frame([{"symbol": "AAA", "date": "2026-09-13", "investability_status": "NEAR_PASS", "tradability_status": "FAIL"}])
        latest = frame([{"symbol": "AAA", "date": "2026-09-14", "investability_status": "NEAR_PASS", "tradability_status": "FAIL", "close_raw": 10}])
