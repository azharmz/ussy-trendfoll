...
def build_feature_store_from_r2(sector_map=None, ready=None, manifest=None) -> dict:
    ...
    # FSE-007 research integration: RS is an exact-date SPY comparison, so an
    # R2 terminal date without an exact SPY row must fail closed before the
    # resulting feature state can reach hard-filter/decision consumers.
