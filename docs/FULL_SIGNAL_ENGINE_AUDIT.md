...
| FSE-013 | HIGH | Effective-date lifecycle state | MISMATCH | Common-date cross-sectional selection is correct, but downstream alert/candidate lifecycle previously conflated current-row signal failure with current-data absence and universe removal. Evidence 10 freezes a three-way contract (`INVALIDATED`, `DATA_UNAVAILABLE`, `OUT_OF_UNIVERSE`). Research correction passed 12/12 contract tests, downstream regression, and independent untouched-validation V2 5/5. Technical validation is closed; explicit production adoption/defer/reject decision remains pending and production is unchanged. |
...
Evidence 10 now closes the technical validation chain for effective-date state semantics on the research branch: common-date selection remains valid; the corrected downstream contract distinguishes `INVALIDATED`, `DATA_UNAVAILABLE`, and `OUT_OF_UNIVERSE`; development/regression gates passed; untouched-validation V1 is retained as an invalid-harness record; corrected frozen V2 passed 5/5 without implementation changes between V1 diagnosis and V2 execution. Production adoption remains an explicit governance decision, not an automatic consequence of validation.

Evidence 11 closes the empirical corporate-action checks for RS, liquidity, and nominal price semantics: adjusted-close RS is appropriate for corporate-action-adjusted return comparison; nominal raw-close price floor remains a valid explicit USSY definition; raw-share-volume windows are corporate-action sensitive and require a correction specification. Evidence 12 identifies the Sep-14 mass terminal-volume anomaly as an upstream READY completeness defect rather than a TrendFoll percentile-formula defect. The post-refresh Sep-15 rerun validates new-session finalization behavior but does not close historical same-date reconciliation.

## Next audit milestone

Follow `docs/FULL_SIGNAL_ENGINE_AUDIT_PROGRESS.md` in order. Immediate remaining governance includes historical Sep-14 READY reconciliation, corporate-action liquidity correction specification, feature methodology/threshold evidence, checklist synchronization, and explicit production decisions. Production remains untouched.
